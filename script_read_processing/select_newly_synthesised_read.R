...
function_sample <- function(target_id, input_folder, output_folder, core, SNP, quality_filter = 45, end_dist = 0, target_mut_filter_rate = 0.3) {
    ...
    tmp_filter_mut_num = tmp %>% filter(!(READ_NAME %in% filtered_reads$READ_NAME))
    
    if(nrow(tmp_filter_mut_num) == 0) {
        return(-1)
    }    
    
    mut_rate = length(unique((tmp_filter_mut_num %>% filter((FLAG == 0 & REF == "T" & BASE == "C") | (FLAG == 16 & REF == "A" & BASE == "G")))$READ_NAME)) / length(unique(ori_test$READ_NAME))     
